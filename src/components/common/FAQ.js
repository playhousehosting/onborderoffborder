...
const FAQ = () => {
  ...
  const faqCategories = [
    ...
    {
      category: 'Advanced Intune Management (NEW - Enterprise Features)',
      icon: CogIcon,
      questions: [
        {
          id: 'advanced-intune-overview',
          question: 'What are the new Advanced Intune Management features?',
          answer: 'We\'ve added 9 enterprise-grade features (Phases 1-9) that transform basic Intune management into a comprehensive policy lifecycle platform: **Phase 1-2: Backup & Migration** - Export/import policies between tenants with conflict resolution. **Phase 3: Comparison Engine** - Detect configuration drift and generate diff reports. **Phase 4: Documentation Generator** - Create professional docs in HTML/Markdown/JSON. **Phase 5: Bulk Clone** - Clone policies with pattern-based naming transformations. **Phase 6: ADMX Import** - Convert Windows Group Policies to Intune policies. **Phase 7: Assignment Analytics** - Detect conflicts and analyze coverage. **Phase 8: Registry Settings** - Create policies via OMA-URI with .reg file import. **Phase 9: Script Management** - Deploy PowerShell/Shell scripts with execution tracking. All features are production-ready, use the Convex proxy architecture, and include comprehensive error handling and audit logging.'
        },
        {
          id: 'backup-migration',
          question: 'How do I backup and migrate Intune policies?',
          answer: '**Backup Process:** Go to Intune Management → Backup & Migration tab. Select policy types to export (14 types supported: Device Configurations, Compliance, Settings Catalog, Security Baselines, Administrative Templates, Apps, App Protection, App Config, Conditional Access, Enrollment Restrictions, Device Categories, Scripts). Click Export → Portal downloads all policies with assignments, settings, and metadata as JSON. Save the JSON file (automatic filename with timestamp). **Import Process:** Upload JSON backup file. Select import mode: **ALWAYS** (import all, skip conflicts), **SKIP** (skip if exists), **REPLACE** (delete and recreate), **UPDATE** (merge changes). Preview shows which policies will be affected. Click Import → Portal processes all policies with progress tracking. Review detailed results (success/failed per policy). **Use Cases:** Migrate dev → test → prod environments, disaster recovery backups, clone tenant configurations, document policy changes, compliance audits. All script content is base64 encoded/decoded automatically.'
        },
        {
          id: 'policy-comparison',
          question: 'How do I compare policies for drift detection?',
          answer: '**Compare Tenant vs Backup:** Export current tenant policies. Upload a previous backup JSON file. Click Compare → Portal generates detailed diff report. View property-level changes (added/removed/modified settings). **Compare Two Backups:** Upload two JSON backup files (e.g., last month vs this month). Click Compare → See what changed between snapshots. Track unauthorized modifications, configuration drift over time. **Report Formats:** **HTML Report** - Color-coded diff with side-by-side comparison, expandable sections for each policy, visual indicators for changes. **Text Report** - Plain text summary for documentation, includes statistics and detailed diff. **Use Cases:** Compliance audits (prove no unauthorized changes), troubleshooting (identify when setting changed), change management (document modifications), security validation (detect malicious policy edits). Reports include policy names, types, all changed settings with before/after values, timestamps, and change statistics.'
        },
        {
          id: 'documentation-generator',
          question: 'How do I generate professional policy documentation?',
          answer: '**Process:** Go to Documentation tab. Select policies to document (multi-select from policy list). Choose format: **HTML** (professional styled report with navigation, best for sharing with stakeholders), **Markdown** (for GitHub/wikis/technical docs), **JSON** (structured data for automation). Optional: Upload ADMX/ADML files to include Group Policy definitions. Click Generate → Portal creates comprehensive documentation. Download instantly. **Documentation Includes:** Policy metadata (name, description, platform, created date, last modified), All settings with current values, Assignment details (which groups receive policy), ADMX/ADML definitions parsed and included (shows policy descriptions, registry keys, allowed values), Settings organized by category for readability. **Use Cases:** Compliance documentation for audits (ISO, SOC2, PCI-DSS), Onboarding documentation for new admins, Change requests and approvals, Disaster recovery runbooks, Stakeholder reporting (show executives what policies are in place). HTML reports are beautifully formatted and can be printed to PDF.'
        },
        {
          id: 'bulk-clone',
          question: 'How do I clone multiple policies at once?',
          answer: '**Bulk Clone Process:** Go to Bulk Clone tab. Select source policies (multi-select from list). Choose transformation mode: **Prefix** (e.g., "PROD-" → "PROD-BitLocker", "PROD-Firewall"), **Suffix** (e.g., " - Copy", " - Test"), **Find/Replace** (e.g., replace "Windows 10" with "Windows 11" in all names), **Pattern** (advanced regex-based transformations). Preview transformations before cloning (see old → new names). Click Clone → Portal creates all copies with progress tracking. **Options:** Maintain original assignments or create new assignments, Clone script content (automatically base64 decoded/encoded), Preserve all settings and metadata. **Use Cases:** Create test versions of production policies (Dev/Test/Prod environments), Replicate policies across departments (Sales-BitLocker → Engineering-BitLocker → HR-BitLocker), Clone for different OS versions, Disaster recovery (quickly recreate policies from backups). Supports all 14+ policy types. Average clone time: 2-5 seconds per policy.'
        },
        {
          id: 'admx-import',
          question: 'How do I import Windows Group Policy ADMX files?',
          answer: '**ADMX Import Wizard:** Go to ADMX Import tab. Upload ADMX file (XML policy definitions, e.g., chrome.admx, firefox.admx, custom policies). Upload ADML file (localized descriptions, e.g., chrome.adml for en-US). Portal parses files and validates structure. **Conversion Process:** Portal extracts policy definitions (name, description, registry keys, allowed values, categories). Maps to Intune Settings Catalog or OMA-URI settings. Generates detection rules for registry-based policies. Creates Intune configuration policy with all settings. **Review Settings:** See all imported policies with descriptions, registry paths (HKLM\\Software\\Policies\\...), value types (String, DWORD, QWORD, etc.), allowed values/ranges. **Import to Intune:** Select policies to import. Assign to groups. Click Import → Portal creates Intune policies. **Supported ADMX:** Microsoft (Office, Edge, OneDrive), Third-party (Chrome, Firefox, Adobe, Zoom), Custom in-house ADMX files. **Use Cases:** Migrate legacy Group Policies to Intune, Deploy third-party app policies not in Intune template catalog, Import custom organizational policies, Standardize settings from existing GPOs. Portal handles complex ADMX structures including lists, multi-string values, and presentation elements.'
        },
        {
          id: 'assignment-analytics',
          question: 'How do I analyze policy assignments and conflicts?',
          answer: '**Assignment Analytics Dashboard:** Go to Assignment Analytics tab. Portal fetches all policies and their assignments (8 policy types: Device Configs, Compliance, Settings Catalog, Security Baselines, Apps, App Protection, App Config, Conditional Access). **Summary Cards:** Total Policies deployed, Total Assignments across all policies, Unique Groups targeted, Unassigned Policies count. **Conflict Detection:** Portal analyzes for: **Same Type Conflicts** (multiple policies of same type assigned to same group → last-write-wins behavior, may cause flapping), **Conflicting Intents** (e.g., BitLocker Required + BitLocker Disabled assigned to overlapping groups → blocks compliance). **Conflict Viewer:** Lists all conflicts with severity (ERROR for blocking conflicts, WARNING for potential issues), shows conflicting policies, affected groups, conflict type description. **Coverage Report:** Shows which groups have no policies assigned (security risk), which groups have comprehensive coverage, policy distribution across organization. **CSV Export:** Download assignment matrix (rows = policies, columns = groups, cells = assignment status), use in Excel for pivot tables and analysis. **Use Cases:** Security audits (ensure critical policies cover all devices), Troubleshooting (identify conflicting policies causing issues), Capacity planning (see most-assigned groups), Compliance (prove policy coverage for auditors), Cleanup (identify unused/redundant policies).'
        },
        {
          id: 'registry-settings',
          question: 'How do I manage Windows Registry settings via OMA-URI?',
          answer: '**Registry Settings Manager:** Go to Registry Settings tab. **Create Policy Manually:** Click Create Policy. Enter policy name and description. Add settings one by one: Select registry hive (HKLM, HKCU, HKCR, HKU, HKCC), enter key path (e.g., Software\\Company\\AppSettings), enter value name (e.g., EnableFeature), select value type (String, DWORD, QWORD, Binary, MultiString, ExpandableString), enter value (portal validates based on type). Portal generates OMA-URI: ./Vendor/MSFT/Registry/{hive}/{path}/{value}. Preview OMA-URI before saving. **Import .reg File:** Click Import .reg File. Upload Windows Registry export file (.reg format). Portal parses file and extracts: Hive, key paths, value names, value types, value data. Preview all settings before importing. Click Import → Portal creates Intune policy with all registry settings as OMA-URI configurations. **Export Policy:** Select existing policy. Click Export to .reg → Downloads .reg file that can be imported to Group Policy or merged on local machines. **Use Cases:** Deploy custom app configurations not available in Settings Catalog, Replicate Group Policy registry settings, Configure third-party software via registry, Apply organization-specific tweaks, Migrate legacy registry-based policies. All value types supported including complex types (REG_MULTI_SZ, REG_EXPAND_SZ, REG_BINARY). Portal validates values before deployment to prevent errors.'
        },
        {
          id: 'script-management',
          question: 'How do I deploy and manage PowerShell and Shell scripts?',
          answer: '**Script Management & Deployment:** Go to Script Management tab. **Create Script:** Click Create Script. Enter script name and description. Select type: **PowerShell (.ps1)** for Windows devices, **Shell (.sh)** for macOS/Linux devices. Paste script content or type directly. Configure execution: **Run as** (System or User context), **Run as 32-bit** (PowerShell only), **Enforce signature check** (PowerShell only). Portal validates syntax (checks for common errors, dangerous commands like Invoke-Expression). Click Create → Portal base64-encodes content and uploads to Intune. **Import Scripts:** Click Import Scripts. Select multiple .ps1 or .sh files. Portal detects type from extension. Bulk imports all scripts with progress tracking. **Script Viewer:** Click any script → View full code in read-only editor, see metadata (type, run context, file name). **Run States Dashboard:** Click Run States for any script. See execution summary: Total Devices targeted, Success count (green), Failed count (red), Pending count (yellow). View detailed device list: Device name, status, last run time, error messages. **Clone Script:** Select script → Click Clone. Enter new name → Creates duplicate with same settings. **Export Script:** Click Export → Downloads .ps1 or .sh file. **Use Cases:** Deploy PowerShell scripts for Windows configuration, Deploy Shell scripts for macOS/Linux management, Automate remediation tasks, Configure applications via scripts, Collect diagnostic information, Schedule maintenance tasks. All script content is base64 encoded for secure transport. Portal tracks execution status on every targeted device with detailed error logging.'
        }
      ]
    },
    {
